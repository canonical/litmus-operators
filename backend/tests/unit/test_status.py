...
def test_blocked_if_no_database_integration(ctx, event, backend_container, leader):
    # GIVEN a running container
    # AND no database relation
    # WHEN we receive any event
    state_out = ctx.run(event, State(containers={backend_container}, leader=leader))
...
def test_waiting_if_no_database_config(
    ctx, event, database_relation, backend_container
):
    # GIVEN a running container
    ...
    # WHEN we receive any event
    state_out = ctx.run(
        event, State(containers={backend_container}, relations={database_relation})
    )
