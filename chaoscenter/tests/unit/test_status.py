...
def test_blocked_if_no_database_integration(ctx, event, nginx_container, leader):
    # GIVEN a running container
    # AND no database relation
    # WHEN we receive any event
    state_out = ctx.run(event, State(containers={nginx_container}, leader=leader))
...
def test_waiting_if_no_database_config(ctx, event, database_relation, nginx_container):
    # GIVEN a running container
    # AND a database relation
    # AND remote hasn't sent any data yet
    # WHEN we receive any event
    state_out = ctx.run(
        event, State(containers={nginx_container}, relations={database_relation})
    )
